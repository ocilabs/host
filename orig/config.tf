...
data "oci_identity_compartment" "service" { id        = var.resident.id}
...
data "oci_core_instance_credentials" "host" {
  #count       = var.options.resource_platform != "linux" ? var.options.count : 0
...
locals {
  # naming conventions
  display_name  = "${data.oci_identity_compartment.service.name}_${var.options.name}"
  dns_label     = "${format("%s%s%s", lower(substr(split("_", data.oci_identity_compartment.service.name)[0], 0, 3)), lower(substr(split("_", data.oci_identity_compartment.service.name)[1], 0, 5)), substr("${var.options.name}", 0, 2))}"
  ...
  merged_freeform_tags = merge(local.module_freeform_tags, var.config.freeform_tags)
  shapes_config = {
...
module "compose_host" {
  ...
  host = {
    shape = var.options.shape
    image = var.options.image
    disk  = var.options.disk
    nic   = var.options.nic
  }
}
